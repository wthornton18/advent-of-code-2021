...
impl Floor {
    ...
    pub fn get_lowest_indexes(&self) -> Vec<(usize, usize)> {
    ...
    pub fn get_basin_sizes(&self) -> i32 {
        let mut values: Vec<i32> = Vec::new();
        for (x, y) in self.get_all_indexes() {
            if self.is_lowest(x, y) {
                values.push(self.get_basin(x, y) as i32);
            }
        }
        let mut start = 1;
    ...
    pub fn get_basin(&self, x: usize, y: usize) -> usize {
        ...
        let mut y = y;
        loop {
            let all_surrounding_indexes = self.get_surrounding_indexes(x, y);
            let surrounding_indexes = all_surrounding_indexes
                                            .clone()
                                            .into_iter()
